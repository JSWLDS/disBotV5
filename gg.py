...
admin = 816847901065740318 


seoul_timezone = timezone('Asia/Seoul')
target_date = datetime(2023, 12, 2)
...
async def on_ready():
    ...
    # bot_database.db에 money 테이블 생성 또는 연결
    conn_bot = sqlite3.connect('bot_database.db')
    cursor_bot = conn_bot.cursor()
    cursor_bot.execute('''CREATE TABLE IF NOT EXISTS money (
    ...
    #channel_id = 1174939056090648576
    #channel = bot.get_channel(channel_id)
    
    #message = '```운명봇 출근...했습니당...ㅠㅡㅠ```'

    #if channel:
        #await channel.send(message)
        
async def update_remaining_time():
    channel_id = 1035448757380194305
    channel = bot.get_channel(channel_id)
    if channel:
        get_remaining_time(channel)
...
@bot.command(name='운명봇안녕')
async def get_remaining_time2(ctx):
    
    message = '```안녕하십니까!! ' + ctx.author.name +'님!!```'
...
async def DM(ctx, user: discord.User, *, message: str = ""):
    ...
    try:
        if user is not None:
            ...
            await ctx.message.delete()  # 사용자의 명령어 삭제
        else:
            await ctx.send('해당 사용자를 찾을 수 없습니다.')
    except discord.Forbidden:
        await ctx.send('DM을 보낼 수 없습니다. 사용자의 DM이 꺼져있을 수 있습니다.')


@bot.command(name='귓이름')
async def DM(ctx, username, *, message):
    ...
    print('------------------------------')
    try:
        # 서버에서 사용자 이름으로 사용자를 찾기 위해 대소문자를 일치시킴
        username = username.lower()
        found_user = None
        for member in ctx.guild.members:
            if member.display_name.lower() == username or member.name.lower() == username:
                found_user = member
                break
        
        if found_user:
            await found_user.send(message)
            await ctx.message.delete()  # 사용자의 명령어 삭제
        else:
            await ctx.send('해당 사용자를 찾을 수 없습니다.')
    except AttributeError:
        await ctx.send('명령어를 사용할 서버에서만 작동합니다.')
    except discord.Forbidden:
        await ctx.send('DM을 보낼 수 없습니다. 사용자의 DM이 꺼져있을 수 있습니다.')
...
@bot.command(name='요일')
async def get_day(ctx):
    now = datetime.utcnow()
    day_of_week = now.strftime('%A')  # 현재 요일을 문자열로 얻음
    # 영어로 된 요일을 한국어로 변환
    if day_of_week == 'Monday':
        ...
        day_of_week_kr = '알 수 없음'

    await ctx.send(f'오늘은 {day_of_week} : {day_of_week_kr}입니다.')


@bot.command(name='태그')
async def tag(ctx, user_id: int):
    user = await bot.fetch_user(user_id)
    if user is not None:
        await ctx.send(f'{user.mention}님을 태그했습니다!')
    else:
        await ctx.send('해당 아이디의 사용자를 찾을 수 없습니다.')
...
async def on_message(message):
    ...
    if isinstance(message.channel, discord.DMChannel):
        ...
        print(f'사용자 {message.author.name}로부터 DM 받음: {message.content}')
        print('------------------------------')
...
@bot.command(name='운명봇인사')
async def getK(ctx): 
    messageK = '```오늘 하루도 화이팅 하십쇼!!!\n화이팅!!!!!!```'
    await ctx.send(messageK)

@bot.command(name='운명봇명령어')
async def getK(ctx): 
    messageK = '```' + '  '+'```'
    await ctx.send(messageK)

@bot.command(name='운명봇자폭해')
async def shutdown(ctx):
    # 사용자 ID를 확인하여 관리자 권한을 가진 사용자만 종료 가능
    if ctx.author.id == admin :
        await ctx.send('```자폭을 실행합니다...```')
        await ctx.send('```.......자폭완료.......```')
        await bot.close()
        return
    else:
        await ctx.send('```관리자권한이 필요합니다.```')
...
async def tutorial(ctx):
    ...
    await ctx.send(embed=discord.Embed(title="운명봇 사용법", description="1. 회원가입 및 로그인 후 !운명 명령어로 오늘의 운명를 확인할 수 있습니다.\n2. 운명은 운명 코인을 사용함으로써 바꿀수 있습니다.\n3. 운명 코인은 출석체크 또는 코인공방에서 제작 성공시 획득할 수 있습니다.\n4. 운명 티켓은 코인공방에 입장할 수 있는 티켓이며 하루 3장씩 부여됩니다.", colour=0x33FF33)) 
    return
...
#     if not user_data:
#         await ctx.send("사용자 데이터가 없습니다.")
#         conn_user.commit()
#         return

#     result_message = "```\n"
#     for user in user_data:
#         result_message += f"User ID: {user[0]}, Last Fortune Date: {user[1]}, Fortune Count: {user[2]}, Fortune: {user[3]}\n"
